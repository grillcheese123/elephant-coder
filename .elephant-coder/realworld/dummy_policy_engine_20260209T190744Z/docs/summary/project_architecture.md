...
- **Ticket**: Implements `WorkItem` with `ticket_id`, `target_channel`, and `data`. Used as concrete work item.
...
2. System retrieves items (e.g., `list_by_channel("email")`).
3. For each item, a `Processor` is selected via `supports()`.
4. `Processor.process(item)` executes and returns a `RunResult`.
5. Results are stored or reported for audit/tracing.